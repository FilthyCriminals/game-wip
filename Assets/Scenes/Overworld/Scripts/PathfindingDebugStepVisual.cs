...
public class PathfindingDebugStepVisual : MonoBehaviour {
        ...
        for (int x = 0; x < grid.GetWidth(); x++) {
            for (int y = 0; y < grid.GetHeight(); y++) {
                ...
                gridSnapshotAction.AddAction(() => {
                    ...
                    Color backgroundColor = Utilities.GetColorFromString("636363");

                    if (isInClosedList) {
                        backgroundColor = new Color(1, 0, 0);
                    }
                    if (isInOpenList) {
                        backgroundColor = Utilities.GetColorFromString("009AFF");
                    }
                    if (isCurrent) {
            ...
            for (int y = 0; y < grid.GetHeight(); y++) {
                ...
                gridSnapshotAction.AddAction(() => { 
                    ...
                    if (isInPath) {
                        ...
                        backgroundColor = Utilities.GetColorFromString("636363");
                    }
    ...
    private void SetupVisualNode(Transform visualNodeTransform, int G, int H, int F) {
        if (F < 1000) {
            visualNodeTransform.Find("GText").GetComponent<TextMeshPro>().SetText(G.ToString());
            visualNodeTransform.Find("HText").GetComponent<TextMeshPro>().SetText(H.ToString());
            visualNodeTransform.Find("FText").GetComponent<TextMeshPro>().SetText(F.ToString());
        } else {
            visualNodeTransform.Find("GText").GetComponent<TextMeshPro>().SetText("");
            visualNodeTransform.Find("HText").GetComponent<TextMeshPro>().SetText("");
            visualNodeTransform.Find("FText").GetComponent<TextMeshPro>().SetText("");
        }
    }
